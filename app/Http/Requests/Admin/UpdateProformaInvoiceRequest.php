...
class UpdateProformaInvoiceRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('proforma-invoices.edit');
    }
    ...
    {
        /** @var ProformaInvoice $proformaInvoice */
        $proformaInvoice = $this->route('proformaInvoice');

        return [
            'invoice_no' => ['required', 'string', 'max:100', Rule::unique('proforma_invoices')->ignore($proformaInvoice)],
            ...
            'items.*.id' => [
                'nullable',
                Rule::exists('proforma_invoice_items', 'id')->where('proforma_invoice_id', $proformaInvoice->id),
            ],
